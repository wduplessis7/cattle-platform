...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      // Look up the FarmUser for this user and embed farm context
      if (token.userId) {
    ...
    async session({ session, token }) {
      if (token) {
        ...
        (session as unknown as Record<string, unknown>).farmRole =
          token.farmRole;
        (session as unknown as Record<string, unknown>).farmSlug =
          token.farmSlug;
      }
      return session;
    },
